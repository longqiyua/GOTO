...
> Statistics is not a pre-filled dashboard. Every value remains zero before activation. After activation, only launches initiated inside GOTO count; the UI shows `—` for unavailable charts until real samples exist and never fills demo data.
