...
统计页与首页共享同一份排序快照，避免“统计有结果、首页为空”的分叉。首页展示规则可拆成以下四个字段：

| 字段 | 来源 | 空数据时的边界处理 |
| --- | --- | --- |
| 当前时段 Top 4 | 当前小时桶的真实启动次数 | 使用最近一次有效时段样本 |
| 相邻时段 Top 1 | 前后相邻小时桶的有效样本 | 取存在数据的一侧，不重复应用 |
| 全时段 Top 6 | 激活后的累计启动次数 | 按已有样本降序补齐 |
| 最近启动时间 | Base 的启动事件 | 无事件时显示统一空状态 |

同一应用在多个时段命中只保留一次，卡片按两行六个槽位排布，不足槽位保持空白而不伪造应用。图标和低饱和应用色与统计排名使用同一应用对象，点击卡片仍走真实启动链路。
