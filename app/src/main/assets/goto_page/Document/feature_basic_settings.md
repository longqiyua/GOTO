...
首页卡片只消费统计结果，不在 Page 内另造一套排序。**最近**由当前时段最常用的四个应用，加上相邻时段最常用的一个应用组成；**常用**取全时段启动次数最高的六个应用。若当前或相邻时段没有独立样本，则沿用已经存在的有效统计结果，直到候选数量达到上限或没有可用数据。

卡片的应用名、启动入口和点击反馈来自同一条应用记录。Android 端优先注入系统真实图标与应用主色，Page 只将主色降饱和、提亮后用于卡片底色；无法读取图标时使用统一的灰色占位，不拼接随机图形。文档中的“查看首页卡片”按钮会同步刷新左侧手机预览，但不会写入统计或修改 Base 契约。
