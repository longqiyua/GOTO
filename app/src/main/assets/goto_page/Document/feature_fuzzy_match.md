...
结果行保留命中来源、候选名称和必要的轻量解释：精确、前缀、拼音、模糊或 Prethink 候选不混成一条无法追溯的“智能结果”。文档按钮可以把指定输入送入左侧预览并高亮对应状态，但不改变 Engine 的分数，也不写入统计。

真实应用的图标和品牌主色由 Kotlin 宿主注入；Page 只生成低饱和浅色底。图标读取失败时统一使用灰色占位块，禁止用随机 emoji 或不相关图标破坏卡片一致性。
