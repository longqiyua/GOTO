...
状态栏属于手机表面的一部分，不再固定使用黑色任务栏色。光感模式切换到夜间时，状态栏读取当前手机背景 token，并以轻微深度差区分安全区；白天则在手机底色上只增加极浅的亮面。状态栏、搜索框和卡片的过渡共用同一组缓动时间，避免顶部突然变黑或出现与手机内部脱节的色带。

亮面方向优先使用系统定位得到的光照角度；没有定位权限时，以时区经度作非阻塞的粗略估算。权限请求只在首次打开进阶模式或光感模式时通过系统弹窗发起，预览端不能伪造授权结果。
