...
class IndexDao(context: Context) {
    ...
    fun upsertFuzzyNode(
        ...
        val childrenJson = JsonCodec.stringMapToJson(children.mapKeys { it.key.toString() })
        val values = ContentValues().apply {
    ...
    fun getFuzzyNodesByType(type: IndexType): List<FuzzyNodeRecord> {
        ...
        cursor.use { c ->
            while (c.moveToNext()) {
                ...
                val children = JsonCodec.jsonToStringMap(childrenJson).mapKeys { it.key.firstOrNull() ?: ' ' }
                list.add(
                    FuzzyNodeRecord(
